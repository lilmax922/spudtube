...
- **Layered caching**: `server/tmdb/cache.ts` is an in-memory TTL cache keyed by operation+args with an injectable clock (`now`) for deterministic tests. `wrap` accepts a fixed TTL or a per-value TTL selector. Layers: `SEARCH_TTL_MS` = 5 min (search/discover/recommendations), `DETAIL_TTL_MS` = 24 h (title/providers/genres), `NOT_FOUND_TTL_MS` = 1 h (negative cache: a Title that returns upstream 404 is mapped to `null` and cached for only 1 h, so graceful degradation doesn't hammer TMDB on every request while a real removal recovers quickly). Transport failures (non-404 errors) are never cached — the loader throws and nothing is stored.
