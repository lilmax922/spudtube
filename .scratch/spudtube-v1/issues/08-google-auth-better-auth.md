...
Implemented on `main` via PR #6 — `feat: Google sign-in via Better Auth (ticket 08)` merged 2026-08-22T16:08:48Z (commit `8c441b1`). Verified merged via `gh pr view 6 --json state,mergedAt`. Summary of decisions:
...
- **Schema**: Better Auth's core tables `session` / `account` / `verification` + `user` in `server/db/schema/auth.ts`, generated from the live `betterAuth` instance via `scripts/generate-auth-schema.mts` (adapter-bundled generator; `@better-auth/cli` lags behind 1.7.x) and explicitly bound to the adapter via the `schema` map. Post-processed for drizzle-orm 0.45 compatibility. Migrations `0002` (initial hand-declared, snake_case via drizzle casing, `account` unique on `(issuer, account_id)`) + `0003` (align indexes/column defaults to generated schema) applied to Docker Postgres. ADR 0004 amended to note the generated source.
- **Mount**: `server/api/auth/[...all].ts` catch-all → `auth.handler(toWebRequest(event))`.
- **Guard (seam S2)**: `server/utils/auth.ts` exports `getAuthSession` and `requireAuthSession` — the single 401 gate every future mutating endpoint calls. Fixture-tested at the route tier against Docker Postgres with **real** user+session rows written via Drizzle and the genuine signed session cookie (`better-auth.session_token` = raw token + HMAC-SHA-256 over `BETTER_AUTH_SECRET`), per S2 ("no hand-rolled tokens, no auth-layer mocks"). `server/utils/auth-fixture.ts` now builds the session through `internalAdapter.createSession` (same path sign-in uses) instead of hand-inserting a row; it is the reusable fixture for tickets 09/10.
- **Client/UI**: `app/lib/auth-client.ts` (`better-auth/vue` `createAuthClient`); `app/components/account-menu.vue` (props `user`, emits `signIn`/`signOut`, S3-testable) wired into `app/app.vue` through `authClient.useSession(useFetch)` so the session hydrates from SSR and clears everywhere on sign-out. i18n keys `auth.signIn` / `auth.signOut` (zh-TW/en). No emoji; Lucide `LogIn`/`LogOut`, dark-system tokens (38px controls, 4px icon gap).
...
Verification evidence: `pnpm typecheck`, `pnpm lint`, `pnpm build` exit 0; full suite 17 files / 66 tests green, stable across repeated runs. Ticket Status corrected from `ready-for-agent` to `ready-for-human` to match the completed-state convention in `01`/`02`/`03`/`11`/`14` (verified 2026-08-23 via `gh pr list --state merged`).
