...
- **My List page** (`app/pages/my-list.vue`): three hand-rolled tabs (Watchlist / Watched / Rated); signed-out visitors see a sign-in prompt with the Google button (same gating as Rating) and the list endpoint is never called anonymously (`immediate: signedIn.value` + `watch`); entries show current poster + name + year and link to their detail routes; the `MyList`/`MyListEntry` response types are shared from the route module via `#server` (no client-side re-declaration); removed references render as a muted "No longer in catalog" row without breaking the tab. Header gains a "My List" link visible only when signed in (`app/app.vue`).
...
Verification evidence: `pnpm typecheck`, `pnpm lint`, `pnpm build` exit 0; full suite 34 files / 148 tests green (Docker Postgres up).
